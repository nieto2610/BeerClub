...
public interface IRecommendationRepository extends JpaRepository<Recommendation, Long> {
    ...
    @Query(value = "SELECT r.id, r.title, r.description, r.create_date, r.image_url, r.is_active, r.product_id  FROM recommendations r\n" +
            "JOIN recommendation_subscription rs on r.id = recommendation_id\n" +
            "JOIN subscriptions s on s.id = rs.subscription_id  \n" +
            "WHERE s.id = :id AND MONTH(r.create_date) = :month AND YEAR(r.create_date) = :year AND r.is_active = true\n" +
